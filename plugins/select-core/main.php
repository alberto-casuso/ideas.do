...
if ( ! function_exists( 'qode_core_activation' ) ) {
	 ...
	 * and defines coney_qodef_core_on_activate action
	 */
	function qode_core_activation() {
		do_action( 'coney_qodef_core_on_activate' );
...
if ( ! function_exists( 'qode_core_theme_menu' ) ) {
	...
	function qode_core_theme_menu() {
		if ( qodef_core_theme_installed() ) {
			...
			coney_qodef_init_theme_options();
			...
			$page_hook_suffix = add_menu_page(
				...
				'coney_qodef_theme_menu',                // The ID used to bind submenu items to this menu
				array(
					$coney_qodef_Framework->getSkin(),
					'renderOptions'
				), // The callback function used to render this menu
				$coney_qodef_Framework->getSkin()->getMenuIcon( 'options' ),             // Icon For menu Item
				$coney_qodef_Framework->getSkin()->getMenuItemPosition( 'options' )            // Position
			);

			foreach ( $coney_qodef_Framework->qodeOptions->adminPages as $key => $value ) {
				$slug = "";
				...
				$subpage_hook_suffix = add_submenu_page(
					...
					array( $coney_qodef_Framework->getSkin(), 'renderOptions' )
				);

				add_action( 'admin_print_scripts-' . $subpage_hook_suffix, 'coney_qodef_enqueue_admin_scripts' );
				add_action( 'admin_print_styles-' . $subpage_hook_suffix, 'coney_qodef_enqueue_admin_styles' );
			};

			add_action( 'admin_print_scripts-' . $page_hook_suffix, 'coney_qodef_enqueue_admin_scripts' );
			add_action( 'admin_print_styles-' . $page_hook_suffix, 'coney_qodef_enqueue_admin_styles' );
		}
	}
