...
class ConeyQodefImport {
    ...
    function init_qode_import() {
        if(isset($_REQUEST['import_option'])) {
            ...
            if($import_option == 'content'){
                $this->import_content('coney_content.xml');
    ...
    public function import_options($file){
        $options = $this->file_options($file);
        update_option( 'qode_options_coney', $options);
    ...
    function qode_admin_import() {
        if (qodef_core_theme_installed()) {
            ...
            $this->pagehook = add_submenu_page(
                ...
                'coney_qodef_theme_menu'.$slug,                // The ID used to bind submenu items to this menu
                array($coney_qodef_Framework->getSkin(), 'renderImport')
            );

            add_action('admin_print_scripts-'.$this->pagehook, 'coney_qodef_enqueue_admin_scripts');
            add_action('admin_print_styles-'.$this->pagehook, 'coney_qodef_enqueue_admin_styles');
        }
    }
    ...
    function qodef_update_options_after_import( $folder ) {
        ...
        foreach ( $demo_urls as $demo_url ) {
            $global_options    = get_option( 'qode_options_coney' );
            $new_global_values = str_replace( $demo_url, $url, $global_options );

            update_option( 'qode_options_coney', $new_global_values );
        }
    }
...
function qode_init_import_object(){
    global $coney_qodef_import_object;
    $coney_qodef_import_object = new ConeyQodefImport();
}

add_action('init', 'qode_init_import_object');

if(!function_exists('coney_qodef_dataImport')){
    function coney_qodef_dataImport(){
        global $coney_qodef_import_object;

        if ($_POST['import_attachments'] == 1)
            $coney_qodef_import_object->attachments = true;
        else
            $coney_qodef_import_object->attachments = false;

        $folder = "coney/";
        if (!empty($_POST['example']))
            $folder = $_POST['example']."/";

        $coney_qodef_import_object->import_content($folder.$_POST['xml']);

        $coney_qodef_import_object->qodef_update_meta_fields_after_import($folder);

        die();
    }

    add_action('wp_ajax_qode_dataImport', 'coney_qodef_dataImport');
}

if(!function_exists('coney_qodef_widgetsImport')){
    function coney_qodef_widgetsImport(){
        global $coney_qodef_import_object;
        ...
        $coney_qodef_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');

        die();
    }

    add_action('wp_ajax_qode_widgetsImport', 'coney_qodef_widgetsImport');
}

if(!function_exists('coney_qodef_optionsImport')){
    function coney_qodef_optionsImport(){
        global $coney_qodef_import_object;
        ...
        $coney_qodef_import_object->import_options($folder.'options.txt');

        $coney_qodef_import_object->qodef_update_options_after_import($folder);

        die();
    }

    add_action('wp_ajax_qode_optionsImport', 'coney_qodef_optionsImport');
}

if(!function_exists('coney_qodef_otherImport')){
    function coney_qodef_otherImport(){
        global $coney_qodef_import_object;
        ...
        $coney_qodef_import_object->import_options($folder.'options.txt');
        $coney_qodef_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');
        $coney_qodef_import_object->import_menus($folder.'menus.txt');
        $coney_qodef_import_object->import_settings_pages($folder.'settingpages.txt');
    ...
    add_action('wp_ajax_qode_otherImport', 'coney_qodef_otherImport');
}