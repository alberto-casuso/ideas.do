<?php
namespace ConeyQodef\Modules\Shortcodes\Icon;

use ConeyQodef\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package ConeyQodef\Modules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface {
    ...
    private function generateIconStyles($params) {
        ...
        if(($params['type'] !== 'qodef-normal' && !empty($params['shape_size'])) || ($params['type'] == 'qodef-normal')) {
            if(!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:'.coney_qodef_filter_px($params['custom_size']).'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if(isset($params['type']) && $params['type'] !== 'qodef-normal') {
            ...
            if(!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: '.coney_qodef_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'height: '.coney_qodef_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'line-height: '.coney_qodef_filter_px($shapeSize).'px';
            }
            ...
            if(!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
				...
				$iconHolderStyles[] = 'border-width: '.coney_qodef_filter_px($params['border_width']).'px';
            ...
            if($params['type'] == 'qodef-square') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: '.coney_qodef_filter_px($params['border_radius']).'px';
                }
            }
        }
