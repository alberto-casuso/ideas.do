...
class PortfolioList implements Lib\ShortcodeInterface {
    ...
    public function render($atts, $content = null) {
	    ...
	    $html = qodef_core_get_shortcode_module_template_part('portfolio', 'portfolio-holder', '', $params, $additional_params);
    ...
    */
	public function getHolderClasses($params){
		...
		$classes[] = 'qodef-pl-gallery';
		$classes[] = !empty($params['space_between_items']) ? 'qodef-pl-'.$params['space_between_items'].'-space' : 'qodef-pl-normal-space';
		...
		if(!empty($params['item_style'])){
			$classes[] = 'qodef-pl-'.$params['item_style'];
		}
		
		if(!empty($params['pagination_type'])){
			$classes[] = 'qodef-pl-pag-'.$params['pagination_type'];
		}
		
		if($params['filter'] === 'yes'){
			$classes[] = 'qodef-pl-has-filter';
		}
		
		if($params['enable_article_animation'] === 'yes'){
			$classes[] = 'qodef-pl-has-animation';
		}
		
		if(!empty($params['enable_variable_width']) && $params['enable_variable_width'] === 'yes'){
			$classes[] = 'qodef-slider-variable-width';
		}
		
		if(!empty($params['navigation_skin'])) {
			$classes[] = 'qodef-nav-'.$params['navigation_skin'].'-skin';
		}
		
		if(!empty($params['pagination_skin'])) {
			$classes[] = 'qodef-pag-'.$params['pagination_skin'].'-skin';
		}
		
		if(!empty($params['pagination_position'])) {
			$classes[] = 'qodef-pag-'.$params['pagination_position'];
		}
	...
	public function getArticleClasses(){
		...
		$article_classes = get_post_class();
