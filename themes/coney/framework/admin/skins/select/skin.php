...
class SelectSkin extends ConeyQodefSkinAbstract {
    /**
     * Skin constructor. Hooks to coney_qodef_admin_scripts_init and coney_qodef_enqueue_admin_styles
     */
	public function __construct() {
		$this->skinName = 'select';

        //hook to admin register scripts
        add_action('coney_qodef_admin_scripts_init', array($this, 'registerStyles'));
        add_action('coney_qodef_admin_scripts_init', array($this, 'registerScripts'));

        //hook to admin enqueue scripts
        add_action('coney_qodef_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('coney_qodef_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('coney_qodef_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('coney_qodef_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
    ...
    public function registerScripts() {
        ...
        foreach ($this->scripts as $scriptHandle => $scriptPath) {
            coney_qodef_register_skin_script($scriptHandle, $scriptPath);
        }
    }
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            coney_qodef_register_skin_style($styleHandle, $stylePath);
        }
	...
	public function renderOptions() {
        ...
        $tab    = coney_qodef_get_admin_tab();
        $active_page = $coney_qodef_Framework->qodeOptions->getAdminPageFromSlug($tab);
     ...
     *
*@param ConeyQodefAdminPage $page current page to load
     *
     * @see QodeSkinAbstract::loadTemplatePart()
     */
