<?php
$author_info_box       = esc_attr( coney_qodef_options()->getOptionValue( 'blog_author_info' ) );
$author_info_email     = esc_attr( coney_qodef_options()->getOptionValue( 'blog_author_info_email' ) );
$author_id             = esc_attr( get_the_author_meta( 'ID' ) );
$social_networks       = coney_qodef_core_plugin_installed() ? coney_qodef_get_user_custom_fields() : false;
$display_author_social = coney_qodef_options()->getOptionValue( 'blog_single_author_social' ) === 'no' ? false : true;
?>
            ...
            <div class="qodef-author-description-content">
                <div class="qodef-author-description-image">
                    <a itemprop="url" href="<?php echo esc_url( get_author_posts_url( $author_id ) ); ?>" title="<?php the_title_attribute(); ?>" target="_self">
						<?php echo coney_qodef_kses_img( get_avatar( get_the_author_meta( 'ID' ), 126 ) ); ?>
                    </a>
                </div>
                            ...
                            <div class="qodef-author-social-icons clearfix">
								<?php foreach ( $social_networks as $network ) { ?>
                                    <a itemprop="url" href="<?php echo esc_url( $network['link'] ) ?>" target="_blank">
										<?php echo coney_qodef_icon_collections()->renderIcon( $network['class'], 'font_elegant' ); ?>
                                    </a>
								<?php } ?>
                            </div>
						<?php } ?>
