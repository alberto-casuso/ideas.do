<?php
namespace ConeyQodef\Modules\Header\Lib;
/**
...
class HeaderConnector {
    ...
    public function connect($config = array()) {
        do_action('coney_qodef_pre_header_connect');
        ...
        if(!empty($config['affect_content'])) {
            add_filter('coney_qodef_content_elem_style_attr', array($this, 'contentMarginFilter'));
        }

        if(!empty($config['affect_title'])) {
            add_filter('coney_qodef_title_content_padding', array($this, 'titlePaddingFilter'));
        }

        do_action('coney_qodef_after_header_connect');
    }
     ...
     *
     * Hooks to coney_qodef_title_content_padding filter
     *
     * @return int
     */
