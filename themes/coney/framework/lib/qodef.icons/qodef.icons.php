...
/*
  Class: ConeyQodefIconCollections
...
class ConeyQodefIconCollections {
	...
	private function initIconCollections() {
		$this->addIconCollection( 'font_awesome', new ConeyQodefIconsFontAwesome( "Font Awesome", "fa_icon" ) );
		$this->addIconCollection( 'font_elegant', new ConeyQodefIconsFontElegant( "Font Elegant", "fe_icon" ) );
		$this->addIconCollection( 'ion_icons', new ConeyQodefIonIcons( "Ion Icons", "ion_icon" ) );
		$this->addIconCollection( 'linea_icons', new ConeyQodefLineaIcons( 'Linea Icons', 'linea_icon' ) );
	}
	...
	public function getSocialIconsMetaBoxOrOption( $attributes ) {
		...
		if ( $type == 'meta-box' ) {
			coney_qodef_create_meta_box_field(
				array(
					...
					'label'         => $label,
					'options'       => array(
						'font_awesome' => esc_html__( 'Font Awesome', 'coney' ),
						'font_elegant' => esc_html__( 'Font Elegant', 'coney' ),
						'ion_icons'    => esc_html__( 'Ion Icons', 'coney' )
					),
					'args'          => array(
						'dependence' => true,
		...
		foreach ( $socialIconCollections as $collection_key => $collection_object ) {
			...
			$qode_icon_pack_container = coney_qodef_add_admin_container(
				array(
					'parent'          => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'coney' ),
	...
	public function getSocialIconWidgetParamsArray() {
		...
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'title'   => esc_html__( 'Icon Pack', 'coney' ),
			'name'    => 'icon_pack',
			'options' => array(
				'font_awesome' => esc_html__( 'Font Awesome', 'coney' ),
				'font_elegant' => esc_html__( 'Font Elegant', 'coney' ),
	...
	public function getIconCollectionIcons( iConeyQodefIconCollection $collection ) {
...
if ( ! function_exists( 'coney_qodef_activate_theme_icons' ) ) {
	function coney_qodef_activate_theme_icons() {
		global $coney_qodef_IconCollections;
		$coney_qodef_IconCollections = ConeyQodefIconCollections::get_instance();
	}

	add_action( 'after_setup_theme', 'coney_qodef_activate_theme_icons' );
}